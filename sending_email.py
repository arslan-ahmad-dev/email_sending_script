...
class EmailSender:
    ...
    def send_email(self, receiver_email):
        try:
            ...
            if server:
                ...
                print(f"❌ Failed to send to {receiver_email}")
        except Exception as e:
            print(f"❌ Unexpected error for {receiver_email}: {e}")
    ...
    def send_emails_from_file(self, email_file):
        ...
        for email in emails:
            ...
            if email:
                self.send_email(email)
                time.sleep(120)  # Wait 2 mins between emails
